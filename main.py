...
with tab1:
    ...
    st.markdown("<div style='height:14px;'></div>", unsafe_allow_html=True)  # jarak kecil
    st.subheader("Total data setelah EDA & Preprocessing")

    # df = data yang sudah drop duplikat + preprocessing + filter kata
    st.metric("Total data (setelah preprocessing)", int(len(df)))

    # opsional: tampilkan berapa yang terbuang dari raw
    ...
    def run_all_training(
        ...
        def tokenize_fn(batch):
            return tokenizer(batch[text_col], truncation=True, max_length=int(max_len))
        ...
        trainer = WeightedTrainer(
            ...
            eval_dataset=val_ds if not fast_mode else None,
        ...
        X_train_ids = pad_sequences(train_ds["input_ids"], maxlen=int(max_len), padding="post", truncating="post", value=pad_id)
        X_val_ids   = pad_sequences(val_ds["input_ids"],   maxlen=int(max_len), padding="post", truncating="post", value=pad_id)
        X_test_ids  = pad_sequences(test_ds["input_ids"],  maxlen=int(max_len), padding="post", truncating="post", value=pad_id)
